...
export default function VersionLogPage() {
  ...
  const [form, setForm] = useState({ version: "", title: "", body: "", type: "feature" as VersionLogEntry["type"] })
  ...
  const fetchEntries = useCallback(async (menuId: string) => {
    ...
    try {
      const data = await apiFetch<{ data: VersionLogEntry[]; total: number }>(
        `/api/auth/version-log/entries?menu_id=${menuId}&page=1&limit=20`
      )
  ...
  useEffect(() => {
    if (selectedMenuId) fetchEntries(selectedMenuId)
  }, [selectedMenuId, fetchEntries])
  ...
  const openEdit = (entry: VersionLogEntry) => {
    setEditing(entry)
    setForm({ version: entry.version, title: entry.title, body: entry.body, type: entry.type })
    setShowModal(true)
  }
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await apiFetch(`/api/super/version-log/entries/${editing.id}`, { method: "PATCH", body: JSON.stringify(form) })
      } else {
        await apiFetch("/api/super/version-log/entries", {
          method: "POST",
          body: JSON.stringify({ ...form, menu_id: selectedMenuId }),
        })
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {!loading && filtered.sort((a, b) => b.created_at.localeCompare(a.created_at)).map(entry => (
          <div key={entry.id} className="bg-white dark:bg-[#0F0F12] rounded-xl border border-gray-200 dark:border-[#1F1F23] overflow-hidden">
            ...
            {expanded.has(entry.id) && (
              <div className="px-14 pb-4 border-t border-gray-100 dark:border-[#1F1F23]">
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed mt-3 whitespace-pre-line">{entry.body}</p>
              </div>
            )}
          </div>
        ))}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea className="w-full px-3 py-2 text-sm border border-gray-200 dark:border-[#1F1F23] rounded-lg bg-white dark:bg-[#1a1a20] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                  rows={4} value={form.body} onChange={e => setForm(p => ({ ...p, body: e.target.value }))} placeholder="Describe what changed..." />
              </div>
            </div>
