...
import { Plus, Pencil, Trash2, X, Search, Truck, Phone, Globe, Link, Unlink } from "lucide-react"
...
export default function CouriersPage() {
  ...
  // Business-linked couriers
  const [activeTab, setActiveTab] = useState<"super" | "business">("super")
  const [businessCouriers, setBusinessCouriers] = useState<BusinessCourier[]>([])
  const [businessCouriersLoading, setBusinessCouriersLoading] = useState(false)
  const [businessCouriersError, setBusinessCouriersError] = useState<string | null>(null)
  const [linkingCourierId, setLinkingCourierId] = useState("")
  ...
  const fetchCouriers = async () => {
    ...
    setError(null)
    try {
      const res = await apiFetch<{ data: Courier[] } | Courier[]>("/api/super/couriers")
      setCouriers((res as { data: Courier[] }).data ?? (res as Courier[]))
    } catch (e: any) {
  ...
  // ── Business couriers ─────────────────────────────────────────────────────
  const fetchBusinessCouriers = async () => {
    setBusinessCouriersLoading(true)
    setBusinessCouriersError(null)
    try {
      const res = await apiFetch<BusinessCourier[] | { data: BusinessCourier[] }>("/api/business/couriers")
      setBusinessCouriers(Array.isArray(res) ? res : (res as { data: BusinessCourier[] }).data)
    } catch (e: any) {
      setBusinessCouriersError(e.message ?? "Failed to load business couriers")
    } finally {
      setBusinessCouriersLoading(false)
    }
  }
  ...
  useEffect(() => {
    if (activeTab === "business") fetchBusinessCouriers()
  }, [activeTab])
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await apiFetch(`/api/super/couriers/${editing.id}`, { method: "PATCH", body: JSON.stringify(form) })
      } else {
        await apiFetch("/api/super/couriers", { method: "POST", body: JSON.stringify(form) })
      }
      await fetchCouriers()
      setShowModal(false)
    } catch (e: any) {
      setError(e.message ?? "Failed to save courier")
    }
  }
  ...
  const remove = async (id: string) => {
    setError(null)
    try {
      await apiFetch(`/api/super/couriers/${id}`, { method: "DELETE" })
      await fetchCouriers()
    } catch (e: any) {
      setError(e.message ?? "Failed to delete courier")
    }
    setConfirmDelete(null)
  }

  // ── Link courier to business ──────────────────────────────────────────────
  const handleLinkCourier = async () => {
    if (!linkingCourierId) return
    setLinkLoading(true)
    setLinkError(null)
    try {
      await apiFetch("/api/business/couriers/link", {
        method: "POST",
        body: JSON.stringify({ courier_id: linkingCourierId }),
      })
      setLinkingCourierId("")
      await fetchBusinessCouriers()
    } catch (e: any) {
      setLinkError(e.message ?? "Failed to link courier")
    } finally {
      setLinkLoading(false)
    }
  }

  // ── Unlink courier from business ──────────────────────────────────────────
  const handleUnlinkCourier = async (courierId: string) => {
    setUnlinkLoading(true)
    setBusinessCouriersError(null)
    try {
      await apiFetch(`/api/business/couriers/${courierId}`, { method: "DELETE" })
      setUnlinkConfirm(null)
      await fetchBusinessCouriers()
    } catch (e: any) {
      setBusinessCouriersError(e.message ?? "Failed to unlink courier")
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab bar */}
      <div className="flex gap-1 border-b border-gray-200 dark:border-[#1F1F23]">
        <button
          onClick={() => setActiveTab("super")}
          className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === "super" ? "border-indigo-500 text-indigo-600 dark:text-indigo-400" : "border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"}`}
        >
          <Truck className="w-4 h-4" /> All Couriers (Super Admin)
        </button>
        <button
          onClick={() => setActiveTab("business")}
          className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === "business" ? "border-indigo-500 text-indigo-600 dark:text-indigo-400" : "border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"}`}
        >
          <Link className="w-4 h-4" /> My Business Couriers
        </button>
      </div>
      ...
      {activeTab === "super" && (
        <>
          ...
          {loading && <div className="py-16 text-center text-gray-400">Loading couriers...</div>}
      ...
      {/* Business Couriers Tab */}
      {activeTab === "business" && (
        <div className="space-y-4">
          {businessCouriersError && (
            <div className="px-4 py-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm">
              {businessCouriersError}
            </div>
          )}
          ...
          {/* Link courier */}
          <div className="bg-white dark:bg-[#0F0F12] rounded-xl border border-gray-200 dark:border-[#1F1F23] p-4 space-y-3">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Link a Courier to This Business</h3>
            {linkError && (
              <p className="text-xs text-red-600 dark:text-red-400">{linkError}</p>
            )}
            <div className="flex gap-3">
              <select
                value={linkingCourierId}
                onChange={e => setLinkingCourierId(e.target.value)}
                className="flex-1 px-3 py-2 text-sm border border-gray-200 dark:border-[#1F1F23] rounded-lg bg-white dark:bg-[#1a1a20] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <option value="">Select courier to link...</option>
              ...
              <button
                ...
                disabled={!linkingCourierId || linkLoading}
          ...
          {/* Linked couriers list */}
          {businessCouriersLoading && <div className="py-8 text-center text-gray-400 text-sm">Loading...</div>}
          {!businessCouriersLoading && businessCouriers.length === 0 && (
            <div className="py-16 text-center text-gray-400 text-sm">No couriers linked to this business yet.</div>
          )}
          {!businessCouriersLoading && businessCouriers.length > 0 && (
            <div className="space-y-3">
              {businessCouriers.map(bc => (
                <div key={bc.courier_id} className="bg-white dark:bg-[#0F0F12] rounded-xl border border-gray-200 dark:border-[#1F1F23] p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center">
                      <Truck className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
                    </div>
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">{bc.courier_name}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${bc.is_active ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}>
                        {bc.is_active ? "Active" : "Inactive"}
                      </span>
                    </div>
                  </div>
                  <div>
                    {unlinkConfirm === bc.courier_id ? (
                      <div className="flex gap-1 items-center">
                        <button onClick={() => handleUnlinkCourier(bc.courier_id)} disabled={unlinkLoading} className="px-2 py-1 bg-red-500 text-white text-xs rounded disabled:opacity-50">
                          {unlinkLoading ? "..." : "Yes"}
                        </button>
                        <button onClick={() => setUnlinkConfirm(null)} className="px-2 py-1 bg-gray-200 dark:bg-gray-700 dark:text-gray-300 text-xs rounded">No</button>
                      </div>
                    ) : (
                      <button
                        onClick={() => setUnlinkConfirm(bc.courier_id)}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                      >
